...
const cases = [
  { name: "AI Software Development", category: "AI engineering · Finland & Europe", image: "/cases-seo/ai-software-development-finland.webp", alt: "AI software development dashboard concept for Finnish and European businesses by SoftBridge Solutions", slug: "ai-software-development-finland" },
  { name: "Enterprise SaaS Platform", category: "SaaS product development · Nordics", image: "/cases-seo/enterprise-saas-platform.webp", alt: "Enterprise SaaS analytics platform concept for Nordic companies by SoftBridge Solutions", slug: "enterprise-saas-platform" },
  { name: "Healthcare Software", category: "Digital health platform · Nordics", image: "/cases-seo/healthcare-software-nordics.webp", alt: "Healthcare software platform concept for Nordic clinics and health organizations", slug: "healthcare-software-nordics" },
  { name: "Logistics Platform", category: "Fleet and logistics software · Europe", image: "/cases-seo/logistics-software-europe.webp", alt: "Logistics and fleet management software dashboard concept for European operations", slug: "logistics-software-europe" },
  { name: "Manufacturing AI", category: "Predictive maintenance · Industry 4.0", image: "/cases-seo/manufacturing-ai-solutions.webp", alt: "Manufacturing AI and predictive maintenance dashboard concept for European factories", slug: "manufacturing-ai-solutions" },
  { name: "Cloud Applications", category: "Cloud-native software · Europe", image: "/cases-seo/cloud-application-development.webp", alt: "Cloud application architecture and observability platform concept by SoftBridge Solutions", slug: "cloud-application-development" },
  { name: "Mobile Product", category: "iOS and Android development · Finland", image: "/cases-seo/mobile-app-development-finland.webp", alt: "Mobile app development concept for Finnish businesses by SoftBridge Solutions", slug: "mobile-app-development-finland" },
  { name: "Retail AI Automation", category: "Commerce intelligence · Europe", image: "/cases-seo/retail-ai-automation.webp", alt: "Retail AI automation and commerce analytics dashboard concept for European brands", slug: "retail-ai-automation" },
  { name: "Digital Transformation", category: "Enterprise modernization · Nordics", image: "/cases-seo/digital-transformation-platform.webp", alt: "Digital transformation platform concept for Nordic enterprise organizations", slug: "digital-transformation-platform" },
] as const;
...
export function LayeredHomepage() {
  ...
  useGSAP(() => {
    ...
    gsap.utils.toArray<HTMLElement>(".layer-card").forEach((card, index) => {
      ...
      gsap.fromTo(
        ...
        { y: 72, opacity: 0 },
        {
          ...
          opacity: 1,
          ease: "power3.out",
          scrollTrigger: { trigger: card, start: "top 76%", end: "top 30%", scrub: 0.9 },
        },
      );
    });
    ...
    gsap.utils.toArray<HTMLElement>(".layer-card").forEach((card, index) => {
      ...
      gsap.fromTo(
        background,
        { yPercent: index % 2 === 0 ? -4 : -2, scale: 1.08 },
        {
          yPercent: index % 2 === 0 ? 5 : 4,
          scale: 1.12,
          ease: "none",
          scrollTrigger: { trigger: card, start: "top bottom", end: "bottom top", scrub: 1.6, invalidateOnRefresh: true },
        },
      );
    });
    ...
    gsap.fromTo(
      ...
      { yPercent: 38, opacity: 0, filter: "blur(10px)" },
      {
        ...
        filter: "blur(0px)",
        ease: "power3.out",
        scrollTrigger: { trigger: ".selected-cases", start: "top 94%", end: "top 24%", scrub: 2.25 },
      },
    );
    ...
    gsap.utils.toArray<HTMLElement>(".case-card").forEach((card, index) => {
      ...
      timeline
        .fromTo(
          ...
          { x: 0, y: 0, opacity: 1, scale: 1, rotate: 0, filter: "blur(0px)", duration: 0.9, ease: "power2.out" },
          0,
        )
        .fromTo(
          image,
          { scale: 1.2, xPercent: index % 3 === 0 ? -5 : index % 3 === 2 ? 5 : 0, yPercent: 8 + (index % 2) * 3 },
          { scale: 1.04, xPercent: 0, yPercent: 0, duration: 0.82, ease: "power2.out" },
          0.03,
        )
        .fromTo(meta, { y: 28, opacity: 0 }, { y: 0, opacity: 1, duration: 0.48, ease: "power2.out" }, 0.42);

      gsap.to(image, {
        yPercent: -5 - (index % 3) * 1.5,
        scale: 1,
        ease: "none",
        scrollTrigger: { trigger: card, start: "top 88%", end: "bottom 4%", scrub: 2.45 + index * 0.08 },
      });
    });
      ...
      <section className="layer-stack" aria-label="Layered content">
        {layers.map((layer, index) => (
          <article className={`layer-card layer-card--${index + 1}`} key={layer.title} style={{ zIndex: index + 1 }}>
            <div className="layer-background" style={{ backgroundImage: `url(${layer.image})` }} aria-hidden="true" />
            <div className="layer-copy">
              <span className="eyebrow">{layer.eyebrow}</span>
              <h2>{layer.title}</h2>
              <p>{layer.body}</p>
              <a href="#cases" className="text-link">Discover more <ArrowUpRight size={15} /></a>
            </div>
            <div className="layer-visual layer-visual--image">
              <figure className="layer-image-frame">
                <Image src={layer.image} alt={layer.alt} fill priority={index === 0} sizes="(max-width: 800px) 100vw, 50vw" className="layer-image" />
                <div className="layer-image-frame__overlay" aria-hidden="true" />
                <figcaption className="layer-image-frame__caption">
                  <span>{String(index + 1).padStart(2, "0")}</span><strong>{layer.accent}</strong>
                </figcaption>
                <div className="layer-image-frame__meta" aria-hidden="true">
                  <span>SOFTBRIDGE SOLUTIONS</span><span>FINLAND / EUROPE</span>
                </div>
              </figure>
            </div>
          </article>
        ...
        <div className="selected-cases__gallery">
          {cases.map((item) => (
            <article className="case-card" key={item.name} itemScope itemType="https://schema.org/CreativeWork">
              <Link id={item.slug} href={{ pathname: `/cases/${item.slug}` }} prefetch scroll className="case-card__link" itemProp="url" aria-label={`Open ${item.name} case study`}>
                <div className="case-card__media">
                  <Image className="case-card__image" src={item.image} alt={item.alt} fill sizes="(max-width: 760px) 100vw, (max-width: 1100px) 50vw, 33vw" itemProp="image" />
                </div>
