...
export function CookieConsent() {
  ...
  return <aside role="dialog" aria-label="Cookie preferences" aria-live="polite" className="fixed inset-x-3 bottom-3 z-[100] mx-auto max-w-3xl border border-white/15 bg-[#0c1412] p-5 text-white sm:p-6">
    <div className="grid gap-5 sm:grid-cols-[1fr_auto] sm:items-end">
      ...
      <div className="flex flex-wrap gap-2"><button onClick={()=>choose("essential")} className="border border-white/25 px-4 py-2.5 text-sm">Essential only</button><button onClick={()=>choose("all")} className="bg-aurora px-4 py-2.5 text-sm font-semibold text-black">Allow analytics</button></div>
    </div>
  </aside>;
}
